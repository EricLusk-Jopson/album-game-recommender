...
For 6 participants this produces 30 turns, scheduled one per week by default.
...
Both scripts accept the same three optional flags.
...
node src/eulerian.js --start "Eric T" --date 2025-04-01
...
# With a custom starting player
python src/eulerian.py --start "Eric T" --date 2025-04-01
...
- The circuit is randomised on each run — the schedule will differ every time you run the script, though it always satisfies the coverage and ABA constraints
- The algorithm makes up to 500 attempts to find a circuit with zero ABA violations; this succeeds in practice for groups up to at least 10 participants
- If `--date` is omitted, turns are printed without dates
- Turn dates are zero-indexed from the start date: turn 1 falls on the start date, turn 2 one week later, and so on
